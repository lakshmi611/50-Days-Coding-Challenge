...
(1, 'lorianthony', 'wvalenzuela@hotmail.com', '2024-05-12'),
...
(1, 7, 'Control but guy whom sometimes nice pressure get.', '2025-06-25 00:41:37'),
...
(5, 4, 'Under war check though past herself.', '2025-03-20 11:52:40'),
...
AND u.user_id IN (SELECT user_id FROM Likes);
...
BEGIN
    ...
    SELECT 
        ...
        -- 2. Total likes given by user
        (SELECT COUNT(*) 
         FROM Likes 
         WHERE user_id = userId) AS total_likes_given,

        -- 3. Total likes received on user's posts
        (SELECT COUNT(*) 
         FROM Likes l
         JOIN Posts p ON l.post_id = p.post_id
         WHERE p.user_id = userId) AS total_likes_received,

        -- 4. Total comments made by user
        (SELECT COUNT(*) 
         FROM Comments 
